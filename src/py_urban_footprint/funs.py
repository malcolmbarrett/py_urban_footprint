...
def gt_summarize_harvest(df):
    ...
    """
    harvest = df.group_by("FarmType").agg(pl.col("Harvested").mean()).sort("Harvested")
...
def gt_summarize_water_by_solar(farms):
    ...
    """
    water_use_by_solar = (
        farms.filter(pl.col("SolarPanels_bin") != "")
        .group_by(pl.col("SolarPanels_bin"))
        .agg(pl.col("WaterUse_GWP_2019").mean().alias("water_use"), pl.len().alias("n"))
        .sort("water_use")
    )
...
def plot_water_usage(farms):
    ...
    Returns:
        a seaborn plot
    """
    plot_data = farms.filter(pl.col("SolarPanels_bin") != "").with_columns(
